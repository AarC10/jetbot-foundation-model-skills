...
MotorDriverNode::MotorDriverNode() : Node("motor_driver_node") {
    if (!pca9685.setPwmFrequency(0)) {
        RCLCPP_ERROR(this->get_logger(),
                     "Failed to set PWM frequency on PCA9685");
    }

    cmdVelSub = this->create_subscription<geometry_msgs::msg::Twist>(
        "cmd_vel", 10,
        std::bind(&MotorDriverNode::cmdVelCallback, this,
                  std::placeholders::_1));

    motorStatusPub =
        this->create_publisher<geometry_msgs::msg::Twist>("motor_status", 10);
}

MotorDriverNode::~MotorDriverNode() {}

void MotorDriverNode::cmdVelCallback(
    ...
    const double leftClamped =
        std::max(-static_cast<double>(MAX_SPEED),
                 std::min(static_cast<double>(MAX_SPEED), leftSpeed));
    const double rightClamped =
        std::max(-static_cast<double>(MAX_SPEED),
                 std::min(static_cast<double>(MAX_SPEED), rightSpeed));

    const uint16_t leftPwm =
        static_cast<uint16_t>((leftClamped / MAX_SPEED + 1.0) / 2.0 * MAX_PWM);
    const uint16_t rightPwm =
        static_cast<uint16_t>((rightClamped / MAX_SPEED + 1.0) / 2.0 * MAX_PWM);

    if (!pca9685.setPwm(leftMotor.pwm, 0, leftPwm)) {
        RCLCPP_ERROR(this->get_logger(), "Failed to set PWM for left motor");
    } else if (!setDirection(leftClamped > 0   ? FORWARD
                             : leftClamped < 0 ? BACKWARD
                                               : STOP,
                             leftMotor)) {
        RCLCPP_ERROR(this->get_logger(),
                     "Failed to set direction for left motor");
    }

    if (!pca9685.setPwm(rightMotor.pwm, 0, rightPwm)) {
        RCLCPP_ERROR(this->get_logger(), "Failed to set PWM for right motor");
    } else if (!setDirection(rightClamped > 0   ? FORWARD
                             : rightClamped < 0 ? BACKWARD
                                                : STOP,
                             rightMotor)) {
        RCLCPP_ERROR(this->get_logger(),
                     "Failed to set direction for right motor");
    }
...
bool MotorDriverNode::setDirection(const MotorDirection direction,
